...
> **Esta é a fronteira humana write-only (CLAUDE.md):** o agente NÃO aplica a migration, nem via CLI de auto-apply, nem via MCP. Após aplicar no SQL Editor (incluindo a segunda execução de idempotência) e rodar as 13 verificações acima, **cole o resultado** para fechar este checkpoint.
