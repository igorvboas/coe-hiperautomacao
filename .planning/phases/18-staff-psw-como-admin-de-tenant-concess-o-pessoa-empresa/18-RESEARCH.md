...
Diretivas acionáveis extraídas do `CLAUDE.md` do projeto — o planner deve verificar conformidade item a item:
...
|------------|-----------------|-------------------|----------|
| "Esta pessoa é admin deste tenant?" | **Banco (RLS / `is_tenant_admin_of()`)** | Servidor (`isTenantAdminOf()`) | O bloqueio real é o banco (CLAUDE.md §1). O servidor espelha para decidir UI/roteamento, nunca para autorizar sozinho — o mesmo par `isPlatformAdmin()`/`is_platform_admin()` já existe |
...
| Contagem "quantas oportunidades perde ao revogar" | **Servidor (runtime)** | — | Derivado; nunca persistido (CLAUDE.md §3) |
| Guard de `/admin/*` | **Servidor (`app/(app)/admin/layout.tsx`)** | Banco | Já existe; `/admin/staff` herda sem plumbing novo |

**Erro de camada a evitar:** implementar "o staff-admin vê o tenant A" filtrando no servidor (`.eq('tenant_id', …)` a partir da lista de concessões) em vez de na RLS. Passaria em toda demo e vazaria na primeira query que esquecesse o filtro — é literalmente o que a regra nº 1 do `CLAUDE.md` proíbe.
...
| `next` | `16.2.6` | App Router, Server Components, Server Actions | Stack decidida no `CLAUDE.md` |
...
// (mesma regra do score — CLAUDE.md §3).
```
...
- **Filtrar por concessão no servidor em vez da RLS.** Viola CLAUDE.md §1.
...
- `.planning/phases/18-*/18-CONTEXT.md`, `.planning/ROADMAP.md` (§Phase 18), `.planning/REQUIREMENTS.md` (GRANT-01..10), `CLAUDE.md`
