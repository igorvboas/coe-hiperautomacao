...
- **Substituir a policy de tenant existente em vez de adicionar uma nova** — quebra D-09 e a regra nº1 do CLAUDE.md. Toda policy nova tem sufixo `_psw_staff` (ou `_atribuicao`), nunca reescreve `opportunities_select`/`opportunities_update` etc.
...
|---------|-------------|-------------|-----|
| "Quem pode ver essa linha por atribuição" | Uma verificação em cada Server Action (JS) | Helper SQL `security definer stable` + RLS | RLS é o único bloqueio que sobrevive a um bug de app esquecendo o filtro — é a regra nº1 do CLAUDE.md, não uma preferência de estilo |
