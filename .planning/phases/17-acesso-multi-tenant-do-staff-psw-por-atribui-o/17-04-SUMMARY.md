...
- **Severidade, com precisão — nem inflada nem minimizada:** isto **não exfiltra dado do tenant vítima** (o conteúdo da nota/risco pertence a quem escreveu, não ao tenant da oportunidade). É um problema de **integridade/poluição de dados**, mais um oráculo fraco de existência de `id` de oportunidade (confirma que um `opportunity_id` existe, sem revelar seu conteúdo). Não é um vazamento de confidencialidade no sentido do CLAUDE.md §1.
