...
- **Achado novo desta sessão, relevante para o fechamento da fase**: o seed de teste do Vitest (`tests/setup/seed-test-tenants.ts`) usa fixtures — tenant `11111111-1111-1111-1111-111111111111` e profile `aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa` — que **já existem no mesmo projeto Supabase de PRODUÇÃO** onde a `0040` foi aplicada (confirmado durante o smoke 7: o profile `psw_staff` impersonado viu 33 oportunidades no tenant do próprio profile, via a policy por tenant pré-existente). Isso significa que popular `.env.test` apontando para este mesmo projeto seria **perigoso** — a suíte de segurança cria e apaga tenants/profiles em `beforeAll`/`afterAll`. A pendência de `.env.test` (débito de Phase 7.5) provavelmente exige um projeto Supabase **separado**, dedicado a teste, e não apenas popular o arquivo `.env.test` com as credenciais de produção. Registrado como observação para o Plan 17-05 e para o fechamento da fase (`.planning/WINDOWS.md` #8).
